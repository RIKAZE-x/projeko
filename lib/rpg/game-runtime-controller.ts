...
import { beginInteraction, closeInteraction } from './world-interaction-controller';
...
export function applyRuntimeEvent(state: GameRuntimeState, event: RuntimeEvent): GameRuntimeState {
  switch (event.type) {
    case 'interaction-start':
      return patchGameRuntimeState(state, { interaction: beginInteraction(state.interaction, event.payload as DungeonAction) });
...
export function updateInteraction(state: GameRuntimeState, action: DungeonAction): GameRuntimeState {
  return patchGameRuntimeState(state, { interaction: beginInteraction(state.interaction, action) });
}
...
export function syncRuntimeToGame(state: GameRuntimeState): GameRuntimeState {
  return {
    ...state,
    game: {
      ...state.game,
