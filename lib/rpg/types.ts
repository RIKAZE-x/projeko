...
export interface Item {
  ...
  history: { kills: number; ownerYears: number; notableEvents: string[] };
  condition: number;
  ownerId?: string;
}
...
export interface Skill {
  ...
  tier: number;
  level: number;
  mastery: 'Beginner'|'Practiced'|'Advanced'|'Perfect'|'Absolute';
  authority: number;
  manaCost: number;
  description: string;
  limitations: string[];
...
export interface Monster {
  ...
  hp:number;
  maxHp:number;
  attributes:Attributes & { THR:number };
  skills:Skill[];
  weaknesses:string[];
...
export interface EconomyState {
  ...
  treasuryReserves:{ gold:number; mana:number };
}
