...
export function resolveCombatAction(game:GameState, combat:CombatState, action:CombatAction, skill?:Skill):CombatResult {
 ...
 else if(action==='skill'){manaCost=Math.max(1,skill?.manaCost??18);if(next.playerMana<manaCost){next.log.unshift('Not enough mana.');return {state:combat,game,xpGained:0,goldGained:0};}next.playerMana-=manaCost;playerDamage=Math.max(2,skillPower(game.character,skill)-Math.floor(enemy.attributes.RES??0));next.enemyStatuses=applyStatus(next.enemyStatuses,{id:'Burn',duration:2,stacks:1,power:Math.max(1,Math.floor(playerDamage/10)),source:skill?.name??'skill'});next.log.unshift(`${skill?.name??'Skill'} deals ${playerDamage} damage and applies Burn.`);}
