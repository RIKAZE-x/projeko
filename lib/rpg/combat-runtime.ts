...
import { inventoryWithLoot } from './equipment-system';
...
export function resolveCombatAction(game:GameState, combat:CombatState, action:CombatAction, skill?:Skill):CombatResult {
 ...
 const enemy=game.activeMonster; let playerDamage=0;
 ...
 else if(action==='attack'){playerDamage=Math.max(1,playerPower(game.character)-Math.floor(enemy.attributes.VIT/4));next.log.unshift(`Attack deals ${playerDamage} damage.`);}
 else if(action==='skill'){const manaCost=Math.max(1,skill?.manaCost??18);if(next.playerMana<manaCost){next.log.unshift('Not enough mana.');return {state:combat,game,xpGained:0,goldGained:0};}next.playerMana-=manaCost;playerDamage=Math.max(2,skillPower(game.character,skill)-Math.floor(enemy.attributes.VIT/5));next.enemyStatuses=applyStatus(next.enemyStatuses,{id:'Burn',duration:2,stacks:1,power:Math.max(1,Math.floor(playerDamage/10)),source:skill?.name??'skill'});next.log.unshift(`${skill?.name??'Skill'} deals ${playerDamage} damage and applies Burn.`);}
 ...
 if(next.enemyHp<=0){next.victory=true;const xp=enemy.rewardXp;const gold=enemy.rewardGold;const material=game.character.equipment[0]?.material??{id:'iron',name:'Iron',hardness:10,durability:20,manaConductivity:5,weight:1};const item=generateLoot(Date.now(),game.character.level,material,'Reward Weapon');const withLoot=inventoryWithLoot(game.character,item);const updated={...game,character:{...withLoot,xp:withLoot.xp+xp,gold:withLoot.gold+gold,role:{...withLoot.role,counters:{...withLoot.role.counters,combatCleared:(withLoot.role.counters.combatCleared??0)+1}}},logs:[`${enemy.name} defeated. +${xp} XP, +${gold} gold, ${item.name} recovered.`,...game.logs].slice(0,20)};return {state:next,game:updated,lootId:item.id,xpGained:xp,goldGained:gold};}
